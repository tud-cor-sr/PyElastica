...
def plot_video(
    ...
    position_of_rod2 = np.array(plot_params_rod2["position"])
    position_of_cylinder = np.array(plot_params_cylinder["position"]) if plot_params_cylinder is not None else None
    director_of_cylinder = np.array(plot_params_cylinder["director"]) if plot_params_cylinder is not None else None
        ...
        for time in range(1, len(time)):
            ...
            if position_of_cylinder is not None:
                ...
                if cylinder is not None:
                    ...
                    cylinder_axis_points[2, :] = np.linspace(start=-cylinder.length.squeeze() / 2,
